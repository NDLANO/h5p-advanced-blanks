...
export class ClozeController {
  ...
  private bindHighlight(highlight: Highlight) {
    ...
    highlightContainer.html(highlight.text);
  }
  ...
  private bindBlank(blank: Blank) {
    ...
    blankContainer.html(`
      <div class="blank-container">
        ${this.isSelectCloze ? this.createSelectOptions(blank) : `<input type="text" id="${blank.id}" />`}
      </div>
    `);

    blankContainer.find('input').on('keyup', () => this.textTyped(blank)); // Removed 'event'
    blankContainer.find('input').on('blur', () => this.checkBlank(blank, 'blur')); // Removed 'event'
    blankContainer.find('input').on('focus', () => this.focus(blank)); // Removed 'event'
  }
  ...
  private refreshCloze(blank?: Blank) {
    if (blank) {
      ...
      const blankInput = this.jquery.find(`#${blank.id}`);
      blankInput.val(blank.enteredText);
    } else {
      // Update all blanks and highlights (if needed)
      this.cloze.highlights.forEach((highlight) => {
        const highlightContainer = this.jquery.find(`#container_${highlight.id}`);
        highlightContainer.text(highlight.text);
      });
